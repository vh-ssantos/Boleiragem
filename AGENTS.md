...
- **Perfil**: avatar button in the `TopAppBar` (both `GruposPeladaScreen` and `MainScreen`) opens a `ModalBottomSheet` — went through **two reworks same day (2026-09-04)**: first from a 6th bottom-nav tab (only reachable inside a grupo) to a sheet with the form embedded; then, per explicit user feedback ("um menu deve ter botões que levam a abas, não um formulário embutido"), to a **real Nubank-style menu** — the sheet is now just a list of tappable rows ("Meu perfil", "Meu histórico", "Meus grupos", "Redefinir senha" for e-mail/senha accounts only, "Sair da conta"), each opening its own full-screen `Dialog` (`ui/screens/perfil/TelaMeuPerfil.kt` / `TelaMeuHistorico.kt` / `TelaMeusGrupos.kt`). "Sair da conta" lives only in that sheet — the old standalone exit icon on `GruposPeladaScreen`'s top bar is gone. **Verified working with a real Google account** on 2026-09-04: all 3 screens open correctly, perfil data loads, "Meus Grupos" shows the owned grupo with a "Dono" chip, and Fase 3's auto-created `Jogador` was confirmed appearing in a grupo's jogadores list. Tip for next session: when tapping precise UI elements via `adb shell input tap` for testing, use `adb shell uiautomator dump` + pull the XML to get exact element bounds — manual pixel-eyeballing from a screenshot was consistently wrong by 150-350px this session and caused a few accidental taps (including one unwanted sign-out, harmless but avoidable).
...
- **The "update now?" prompt itself was confirmed working end-to-end on 2026-09-04** with the real external tester: they saw the "New version available" dialog, tapped Update, the APK downloaded ("Download complete" notification appeared) — but then the install silently never happened, no error shown anywhere. Root-caused to Android blocking the install because the app lacked the **`REQUEST_INSTALL_PACKAGES`/"instalar apps desconhecidos"** permission for itself (self-update requires this, since `AppUpdateChecker` triggers the install from inside the host app process, not a separate installer app) — Android denies this silently with zero error surfaced to the SDK or the app. The tester granted the OS permission manually and it *still* didn't install, ruling out the simplest fix; the debug keystore fingerprint was confirmed unchanged since 2024-05-02 so it isn't a signature mismatch either. Leading remaining suspect (not yet confirmed): **Samsung's "Bloqueador Automático" (Auto Blocker, One UI 6+)**, which blocks sideloaded installs independent of the per-app unknown-sources toggle — the tester's device is a Samsung. Fixed the *silent* half of this regardless: `AppUpdateChecker.verificarAtualizacao` now checks `packageManager.canRequestPackageInstalls()` before calling `updateIfNewReleaseAvailable()`, and if missing shows a `Toast` + opens `Settings.ACTION_MANAGE_UNKNOWN_APP_SOURCES` directly for this package, instead of the update silently doing nothing. **Confirmed 2026-09-04**: after this fix, opening the app landed the tester directly on the permission screen as expected. **Still open / not yet confirmed**: whether the actual install completes end-to-end after all this (permission granted + possible Auto Blocker exception) — the tester installed the latest build manually meanwhile, next real test needs a fresh newer release to trigger the in-app flow again. Don't tell the user "the auto-update works" until an install is seen completing without manual intervention.
