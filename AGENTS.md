...
## Backlog — requested 2026-09-02, nothing started yet

The user listed this before going to sleep, explicitly asking to just record it, not implement anything. Next session should start by discussing/scoping these with the user, not silently building.

1. **Open architecture question for Fase 2 (cloud content sync)**: does Room get replaced, or stay as an offline cache alongside Firestore? Not decided. Firestore has built-in offline persistence, which could make Room partially redundant for grupo-scoped synced data (jogadores/sorteio/histórico) once Fase 2 ships — but Room still has to exist for guest/"entrar sem conta" users, who have no Firestore identity at all. Whatever Fase 2's design ends up being, it has to keep guest mode working on local-only data. Resolve this explicitly before writing Fase 2 code, don't default into it implicitly.
2. **New "Perfil" tab/screen**: each logged-in user should be able to view and edit their own profile — fields mentioned so far: nome, posição que joga, idade ("etc." — ask for the full field list when scoping this). This overlaps directly with **Fase 3** (tying `Jogador` to a real user account, see the Authentication section above) — a sensible design is probably that this *is* the Fase 3 implementation surface (the profile screen becomes where the user/Jogador link gets created/edited), rather than a separate parallel feature. Worth confirming that framing with the user before starting.
3. **"Histórico" within that profile**: personal stats across peladas — quantas peladas jogou, gols marcados, etc. Note this is **personal/cross-group** stats for the logged-in user specifically, distinct from the existing `HistoricoScreen`/`EstatisticasScreen` which are scoped per-grupo and per-local-`Jogador`-record today. Depends on both Fase 2 (synced content to aggregate across) and Fase 3 (a user having a stable identity to aggregate stats *by*, across devices/grupos) — this is downstream of those, not independent work.
4. **Visual/UX redesign of the whole app**: user's own words — the app so far "foi feito apenas para funcionar e não para ser atraente à vista." This is a broad, unscoped ask (not a specific screen) — treat as its own design pass across the app (Material theming/`ui/theme/`, spacing, empty states, iconography, motion, etc.), not something to fold piecemeal into other feature work. Needs its own scoping conversation (which screens first? any visual reference/inspiration the user has in mind? keep Material 3 or go more custom?) before implementation.
