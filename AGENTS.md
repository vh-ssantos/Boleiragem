...
- **Perfil**: new 6th tab ("Meu Perfil") shipped 2026-09-04 — view/edit nome/posição/idade, "Meu Histórico" (personal cross-grupo stats), sair da conta. Convidado sees a limitation notice instead of the form.
...
- **Not yet verified end-to-end** (needs a real 2nd account/device, not available in the session that shipped this): Fase 2 content sync actually propagating between two devices; the Fase 3 auto-created Jogador; editing/saving on the Perfil screen; "Meu Histórico" actually populating with real data. Everything above only got a single-device/guest-mode smoke test (compiles, installs, no crash, correct empty states). Don't tell the user any of this "works" beyond that until it's been seen succeeding with a real account.
...
2. `MainActivity.kt` — `BoleiragemApp()` is a **hand-rolled state machine** (`showSplashScreen`, `showLoginScreen`, `showGruposScreen`, `showResultadoSorteioRapido`, booleans checked in a `when`) that decides which top-level screen is shown. Inside `MainScreen()`, the 6 main tabs (Jogadores / Regras / Sorteio / Jogo / Estatísticas / **Perfil**, added 2026-09-04) are pages of a `HorizontalPager`, manually wired with a `when(page)` — **this is not driven by `BoleiragemNavHost` at all**, even though `NavDestinations` routes exist for these same screens and `BoleiragemNavHost` has empty composable stubs for them (`/* Conteúdo gerenciado pelo ViewPager */`). `MainScreen` now also takes an `onSairClick` param (threaded from `BoleiragemApp()`, same signOut+state-reset logic as the grupos screen's own "Sair") so the Perfil tab can log out without a 4th navigation mechanism.
